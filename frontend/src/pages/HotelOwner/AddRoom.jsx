...
const AddRoom = () => {
  ...
  const [showRegisterHotel, setShowRegisterHotel] = useState(false);
    ...
    try {
      ...
      if (data?.success) {
        ...
        if (ownerHotels.length > 0) {
          setSelectedHotelId((prev) => prev || ownerHotels[0]._id);
          setShowRegisterHotel(false);
        } else {
          setSelectedHotelId("");
  ...
  const handleRegisterHotel = async (e) => {
    ...
    try {
      ...
      if (data?.success) {
        ...
        setSelectedHotelId(data.hotel._id);
        setShowRegisterHotel(false);
        resetHotelForm();
      } else {
  ...
  const handleUpdateHotel = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.patch(
        `/api/hotels/owner/${selectedHotel._id}`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
      ...
      if (data?.success) {
        setHotels((prev) =>
          prev.map((hotel) => (hotel._id === data.hotel._id ? data.hotel : hotel))
        );
  ...
  const openEditHotelForm = () => {
    if (!selectedHotel) {
      toast.error("Please select a hotel first");
      return;
    }

    setShowEditHotel((prev) => !prev);
    setShowRegisterHotel(false);
    setHotelImage(null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!selectedHotel) {
        toast.error("Please register or select a hotel first");
        return;
      }
      ...
      <div className="bg-white border rounded-2xl p-4 sm:p-6 mt-8 shadow-sm">
        <div className="flex flex-col lg:flex-row gap-4 lg:items-end">
          <div className="flex-1">
            <p className="font-semibold mb-3">Select Hotel</p>
            ...
            >
              {hotels.length === 0 && (
                <option value="">No hotel registered yet</option>
              )}
          ...
          <button
            type="button"
            onClick={() => {
              setShowRegisterHotel((prev) => !prev);
              setShowEditHotel(false);
              resetHotelForm();
            }}
            className="bg-black text-white px-6 py-4 rounded-xl"
          >
            {showRegisterHotel ? "Close Form" : "Register Another Hotel"}
          </button>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setHotelForm((prev) => ({ ...prev, area: e.target.value }))
                }
                className="border p-4 rounded-xl w-full"
              />
            </div>
        ...
        <div className="bg-white border rounded-2xl p-4 sm:p-6 mt-8 shadow-sm">
          <div className="flex flex-col md:flex-row justify-between gap-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
              <img
                src={selectedHotel?.images?.[0] || assets.roomImg}
                alt="hotel"
              ...
              <div>
                ...
                <div className="inline-block mt-4 bg-[#eef9ff] text-[#00ADEF] px-4 sm:px-5 py-2 rounded-full capitalize">
                  {selectedHotel.city} • {selectedHotel.area}
                </div>
              </div>
            </div>
                ...
                <label className="cursor-pointer inline-block">
                  <img
                    src={
                      hotelImage
                        ? URL.createObjectURL(hotelImage)
                        : selectedHotel?.images?.[0] || assets.uploadArea
                    }
                    alt="hotel"
                    className="w-36 h-36 sm:w-40 sm:h-40 object-cover rounded-2xl border"
                  />
