...
const ListRoom = () => {
  // Normalize data so it's safe
  const normalizedRooms = roomCommonData.map(room => ({
    ...room,
    amenities: Array.isArray(room.amenities) ? room.amenities : [],
    isAvailable: typeof room.isAvailable === 'boolean' ? room.isAvailable : false
  }))
            ...
            {rooms.map((item, index) => (
              <tr key={index}>
                ...
                <td className="py-3 px-4 text-gray-700 border-t border-gray-300 max-sm:hidden">
                  {item.amenities.length > 0 ? item.amenities.join(', ') : 'No facilities listed'}
                </td>
                <td className="py-3 px-4 text-gray-700 border-t border-gray-300">
